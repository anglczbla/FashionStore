...
const createPayment = async (req, res) => {
  ...
  if (!req.body.orders_id || !req.body.amount || !req.body.paymentMethod) {
    ...
    return res
      .status(400)
      .json({ message: "orders_id, amount, dan paymentMethod harus diisi." });
  }
...
const updatePayment = async (req, res) => {
  ...
  try {
    ...
    payment.firstName = firstName ?? payment.firstName;
    payment.lastName = lastName ?? payment.lastName;
    payment.address = address ?? payment.address;
    payment.apartment = apartment ?? payment.apartment;
    payment.city = city ?? payment.city;
    payment.province = province ?? payment.province;
    payment.postalCode = postalCode ?? payment.postalCode;
    payment.phone = phone ?? payment.phone;
    payment.saveInfo = saveInfo ?? payment.saveInfo;
